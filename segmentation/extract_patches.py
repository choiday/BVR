...
def get_data_training(IMGC_train_imgs_original,
                      IMGC_train_groudTruth,
                      patch_height,
                      patch_width,
                      N_subimgs,
                      inside_FOV):
    train_imgs_original = load_hdf5(IMGC_train_imgs_original)
    train_masks = load_hdf5(IMGC_train_groudTruth) #masks always the same
...
#Load the original data and return the extracted patches for training/testing
def get_data_testing(IMGC_test_imgs_original, IMGC_test_groudTruth, Imgs_to_test, patch_height, patch_width):
    ### test
    test_imgs_original = load_hdf5(IMGC_test_imgs_original)
...
def get_data_testing_overlap(IMGC_test_imgs_original, IMGC_test_groudTruth, Imgs_to_test, patch_height, patch_width, stride_height, stride_width):
    ### test
    test_imgs_original = load_hdf5(IMGC_test_imgs_original)
    test_masks = load_hdf5(IMGC_test_groudTruth)
...
def is_patch_inside_FOV(x,y,img_w,img_h,patch_h):
    ...
    R_inside = 270 - int(patch_h * np.sqrt(2.0) / 2.0) #radius is 270 (from IMGC db docs), minus the patch diagonal (assumed it is a square #this is the limit to contain the full patch in the FOV
    ...
    for i in range(data_imgs.shape[0]):  #loop over the full images
        for x in range(width):
            for y in range(height):
                if inside_FOV_IMGC(i,x,y,original_imgs_border_masks)==True:
...
def kill_border(data, original_imgs_border_masks):
    ...
    for i in range(data.shape[0]):  #loop over the full images
        for x in range(width):
            for y in range(height):
                if inside_FOV_IMGC(i,x,y,original_imgs_border_masks)==False:
                    data[i,:,y,x]=0.0


def inside_FOV_IMGC(i, x, y, IMGC_masks):
    assert (len(IMGC_masks.shape)==4)  #4D arrays
    assert (IMGC_masks.shape[1]==1)  #IMGC masks is black and white
    # IMGC_masks = IMGC_masks/255.  #NOOO!! otherwise with float numbers takes forever!!

    if (x >= IMGC_masks.shape[3] or y >= IMGC_masks.shape[2]): #my image bigger than the original
        return False

    if (IMGC_masks[i,0,y,x]>0):  #0==black pixels
        # print (IMGC_masks[i,0,y,x]  #verify it is working right)
        return True
    else:
        return False
